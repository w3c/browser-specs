/**
 * Publish an NPM package when pre-release PR is merged, using the commit on
...
async function releasePackage(prNumber) {
  ...
  const match = preReleasePR.title.match(/^📦 Release (.*)@(.*)$/);
  if (!match) {
  ...
  const tmpFolder = fs.mkdtempSync(path.join(os.tmpdir(), `${repo}-`));

  try {
    execSync(`git clone https://github.com/${owner}/${repo}`, {
      cwd: tmpFolder
    });
    const installFolder = path.join(tmpFolder, repo);
    ...
    if (pubResult.version === pubResult.oldVersion) {
      ...
      const rawTag = `${type}@${pubResult.version}`;
